...
void OpenAddressing<K, V>::resizeDown() {
    ...
    if (_alfa <= 0.25) {
        int old_size = _size;
        _size = _size / 2;
        Pair<K, V>* temp = new Pair<K, V>[_size];
        for (int i = 0; i < old_size; i++) {
            if (_table[i]._key != emptyKey) {
                int j = 0;
                while (temp[hash(_table[i]._key, j)]._key != emptyKey) {
...
void OpenAddressing<K, V>::insert(K key, V value) {
    ...
    while (_table[hashIndex]._key != emptyKey && _table[hashIndex]._key != key) {
        i++;
        hashIndex = hash(key, i);
    }
    if (_table[hashIndex]._key != key) {
        _table[hashIndex] = Pair<K, V>(key, value);
        _capacity++;
        resizeUp();
    } else {
        _table[hashIndex]._value = value;
    }
}
...
void OpenAddressing<K, V>::remove(K key) {
    int i = 0;
    int hashIndex = hash(key, i);
    while (_table[hashIndex]._key != key && _table[hashIndex]._key != emptyKey && i < _size) {
        i++;
        hashIndex = hash(key, i);
    }
    if (_table[hashIndex]._key == key) {
...
bool OpenAddressing<K, V>::exists(K key) {
    ...
    return _table[hashIndex]._key == key;
}
